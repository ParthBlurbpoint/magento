...
class Kodematix_Hidecatalog_Model_Observer
{
    /**
    ...
    public function controllerFrontInitBefore(Varien_Event_Observer $observer)
    {
        if (version_compare(Mage::getVersion(), '1.8', '<')) {
            Mage::getConfig()->setNode(
                ...
                'Kodematix_Hidecatalog_Model_Catalog_Resource_Category_Flat17'
            );
        }
    }
    
    /**
     * Add the hidecatalog filter sql to product collections
     *
     * @param Varien_Event_Observer $observer
     * @return void
     */
    ...
    /**
     * Add the hidecatalog filter sql to category collections
     *
     * @param Varien_Event_Observer $observer
     * @return void
     */
    ...
    public function catalogCategoryLoadAfter(Varien_Event_Observer $observer)
    {
        ...
        if ($category->getData('forbidden_by_hidecatalog')) {
    ...
    public function catalogProductLoadAfter(Varien_Event_Observer $observer)
    {
        ...
        if ($product->getData('forbidden_by_hidecatalog')) {
    ...
    protected function _applyHiddenEntityHandling($entityTypeCode)
    {
        ...
        if ($helper->isModuleActive() && !$helper->isDisabledOnCurrentRoute()) {
            ...
            if (Mage::app()->getRequest()->getModuleName() !== 'customer') {
                Mage::helper('kodematix_hidecatalog/hidden')->applyHiddenEntityHandling($entityTypeCode);
            }
        }
    }
    ...
    /**
     * Set the html block cache to disabled/invalid when hidecatalog visibility settings
    ...
    public function catalogCategorySaveAfter(Varien_Event_Observer $observer)
    {
        ...
        if ($helper->isModuleActive($category->getStore(), false) && !$helper->isDisabledOnCurrentRoute()) {
            if ($category->dataHasChangedFor(Kodematix_Hidecatalog_Helper_Data::HIDE_GROUPS_ATTRIBUTE)) {
    ...
    public function customerGroupSaveAfter(Varien_Event_Observer $observer)
    {
        ...
        Mage::app()->cleanCache(array(
            Kodematix_Hidecatalog_Helper_Data::CUSTOMER_GROUP_CACHE_TAG
        ));
    }

    /**
     * Add the hidecatalog filter to the wishlist item collection.
     *
    ...
    /**
     * Add the hidecatalog filter to the product collection count so the numbers
    ...
    /**
     * Switch hidecatalog attribute input to display only if configured to avoid loading
    ...
    /**
     * Switch hidecatalog attribute input to display only if configured
     * 
    ...
    /**
     * "Unload" the specified catalog entity if the hidecatalog settings specify so
     *
    ...
    protected function _applyGroupsCatalogSettingsToEntity(Mage_Catalog_Model_Abstract $entity)
    {
        ...
        if ($helper->isModuleActive() && !$helper->isDisabledOnCurrentRoute()) {
            if (!$helper->isEntityVisible($entity)) {
                ...
                $entity->setData('forbidden_by_hidecatalog', true);
            }
        }
    }

    /**
     * Add the hidecatalog filter sql to catalog collections using the hidecatalog filter resource model
     *
    ...
    {
        return Mage::helper('kodematix_hidecatalog');
    }
    ...
    {
        return Mage::getResourceSingleton('kodematix_hidecatalog/filter');
    }
}
