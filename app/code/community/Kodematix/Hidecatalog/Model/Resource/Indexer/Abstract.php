...
abstract class Kodematix_Hidecatalog_Model_Resource_Indexer_Abstract extends Mage_Index_Model_Resource_Abstract
{
    /**
    ...
    /**
     * Array of all valid group ids plus Kodematix_Hidecatalog_Helper_Data::USE_NONE
     *
     * @var array $_groupIds
     */
    ...
    {
        parent::__construct();
        $this->_helper = Mage::helper('kodematix_hidecatalog');
        $this->_initStores();
        $this->_initGroupIds();
    }
    ...
    protected function _initGroupIds()
    {
        ...
        $this->_groupIds[] = Kodematix_Hidecatalog_Helper_Data::USE_NONE;
    }
    ...
    {
        return 'Kodematix_Hidecatalog::' . $this->_getEntityTypeCode();
    }
    ...
    /**
     * Return the hidecatalog index table name for this indexers entity
     *
     * @return string
     */
    ...
    /**
     * @return Kodematix_Hidecatalog_Model_Resource_Setup
     */
    protected function _getSetupModel()
    {
        return Mage::getResourceModel(
            'kodematix_hidecatalog/setup',
            'kodematix_hidecatalog_setup'
        );
    }
    ...
    protected function _checkAttribute()
    {
        ...
        if (! $attributeChecked) {
            ...
            $select = $this->_getReadAdapter()->select()
                ...
                ->where('attribute_code=?', Kodematix_Hidecatalog_Helper_Data::HIDE_GROUPS_ATTRIBUTE);
    ...
    protected function _reindexEntity($event = null, array $limitToGroupIds = null)
    {
        ...
        $attribute = Mage::getSingleton('eav/config')->getAttribute(
            $this->_getEntityTypeCode(), Kodematix_Hidecatalog_Helper_Data::HIDE_GROUPS_ATTRIBUTE
        );
        $select = $this->_getReadAdapter()->select()
    ...
    {
        // Entities that don't have a value for the hidecatalog_groups attribute
        if (null === $row['group_ids']) {
            $row['group_ids'] = Kodematix_Hidecatalog_Helper_Data::USE_DEFAULT;
            $row['store_id'] = Mage::app()->getStore(Mage_Core_Model_Store::ADMIN_CODE)->getId();
        }

        if (Kodematix_Hidecatalog_Helper_Data::USE_DEFAULT == $row['group_ids']) {
            ...
            $row['orig_group_ids'] = Kodematix_Hidecatalog_Helper_Data::USE_DEFAULT;
            ...
            if (in_array(Kodematix_Hidecatalog_Helper_Data::USE_NONE, $row['group_ids'])) {
                $row['group_ids'] = array();
            }
    ...
    /**
     * Take the hidecatalog attribute value and return an array with valid group ids
     *
    ...
    )
    {
        foreach (array_diff($this->_frontendStoreIds, $storesHandled) as $storeId) {
            if (Kodematix_Hidecatalog_Helper_Data::USE_DEFAULT === $entityDefaultGroupsWithoutMode) {
