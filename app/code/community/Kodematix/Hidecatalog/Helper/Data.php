...
class Kodematix_Hidecatalog_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    const XML_CONFIG_PRODUCT_MODE = 'kodematix_hidecatalog/general/product_mode';
    const XML_CONFIG_CATEGORY_MODE = 'kodematix_hidecatalog/general/category_mode';
    const XML_CONFIG_PRODUCT_DEFAULT_PREFIX = 'kodematix_hidecatalog/general/product_default_';
    const XML_CONFIG_CATEGORY_DEFAULT_PREFIX = 'kodematix_hidecatalog/general/category_default_';
    const XML_CONFIG_DISABLED_ROUTES = 'global/kodematix_hidecatalog/disabled_on_routes';

    const HIDE_GROUPS_ATTRIBUTE = 'hidecatalog_groups';
    const HIDE_GROUPS_ATTRIBUTE_STATE_CACHE = 'hidecatalog_groups_state_cache';
    const CUSTOMER_GROUP_CACHE_TAG = 'hidecatalog_customer_group';
    ...
    /**
     * If set to false hidecatalog filtering is skipped
     *
     * @var bool|null
     */
    ...
    /**
     * Return a configuration setting from within the kodematix_hidecatalog/general section.
     *
    ...
    {
        return Mage::getStoreConfig('kodematix_hidecatalog/general/' . $field, $store);
    }
    ...
    public function isEntityVisible(Mage_Catalog_Model_Abstract $entity, $customerGroupId = null)
    {
        ...
        if (!is_array($groupIds) && !is_string($groupIds)) {
            ...
            $visibility = Mage::getResourceSingleton('kodematix_hidecatalog/filter')
    ...
    public function getIndexTableByEntityType($entityType)
    {
        ...
        switch ($entityType->getEntityTypeCode()) {
            ...
            case Mage_Catalog_Model_Product::ENTITY:
                $table = 'kodematix_hidecatalog/product_index';
                break;

            case Mage_Catalog_Model_Category::ENTITY:
                $table = 'kodematix_hidecatalog/category_index';
                break;
        }
        return $table;
    }
    ...
    /**
     * Return the hidecatalog attribute model
     * 
    ...
    public function getGroupNamesAsString(array $value)
    {
        ...
        $key = array_search(Kodematix_Hidecatalog_Helper_Data::USE_DEFAULT, $value);
        if (false !== $key) {
            $list[] = $this->__(Kodematix_Hidecatalog_Helper_Data::LABEL_DEFAULT);
            unset($value[$key]);
        }
        $key = array_search(Kodematix_Hidecatalog_Helper_Data::USE_NONE, $value);
        if (false !== $key) {
            $list[] = $this->__(Kodematix_Hidecatalog_Helper_Data::LABEL_NONE);
            unset($value[$key]);
        }
        if (count($value)) {
            ...
            $groups->initCache(Mage::app()->getCache(), 'hidecatalog', array(
