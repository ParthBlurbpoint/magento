 ...
 */
class Kodematix_Hidecatalog_Block_Adminhtml_System_Tools_Migration
    extends Mage_Adminhtml_Block_Template
{
    ...
    public function getGroupsCatalogInstallationStatus()
    {
        if (is_null($this->_groupsCatalogInstallationStatus)) {
            ...
            $config = Mage::getConfig()->getNode('modules/Kodematix_GroupsCatalog');
            if ($config) {
    ...
    {
        return (bool)Mage::getStoreConfig('catalog/hidecatalog', Mage_Core_Model_Store::ADMIN_CODE);
    }
    ...
    protected function _getGroupsCatalogAttribute($entityCode)
    {
        if (!isset($this->_attributeModels[$entityCode])) {
            $this->_attributeModels[$entityCode] = Mage::getSingleton('eav/config')
                    ->getAttribute(
                        $entityCode, Kodematix_Hidecatalog_Helper_Migration::GROUPSCATALOG1_ATTRIBUTE_CODE
                    );
        }
    ...
    {
        return 0 < Mage::getResourceSingleton('core/resource')->getDbVersion('hidecatalog_setup');
    }
}
