 ...
 */
class MD_Partialpayment_Block_Adminhtml_Options extends Mage_Adminhtml_Block_Sales_Order_Create_Abstract {
	...
	return $priceLabel;
     ...
     public function getAdminCheckboxLabel($product,$qty = 1) {
	if(empty($product)) {
	    $product = $this->getProduct();
	}
	...
	if (!is_null($option->getInitialPaymentAmount()) || $configInitialPaymentType == MD_Partialpayment_Model_Options::PAYMENT_FIXED) {
	    $initialPaymentAmount = (!is_null($option->getInitialPaymentAmount())) ? $option->getInitialPaymentAmount() : $configInitialPayment;
	    if ($initialPaymentAmount > 0) {
		$priceLabel = sprintf("Pay Now %s and rest with easy installments.", Mage::helper("core")->currency($initialPaymentAmount*$qty, true, false));
	    }
	}
	return $priceLabel;
    }
