...
*/
class MD_Partialpayment_SagepayController extends Mage_Core_Controller_Front_Action
{
    ...
    public function callback3dAction()
    {
        ...
        $sessionMethodCode = Mage::getSingleton('sagepaysuite/session')->getMethodCode(true);
        ...
        $transaction->setPares($pares)
                    ->save();
        $returnUrl = Mage::getUrl('md_partialpayment/summary/view',array('payment_id'=>$paymentsId));
        if(!is_null($sessionMethodCode) && strlen($sessionMethodCode) > 0){
            ...
            if ($result->getResponseStatus() == Ebizmarts_SagePaySuite_Model_Api_Payment::RESPONSE_CODE_APPROVED || $result->getResponseStatus() == 'AUTHENTICATED') {
                ...
                {
                    Mage::getSingleton("core/session")->addError($result->getErrorMessage());
                }
                if($result->getSuccessMessage())
                {
                    Mage::getSingleton("core/session")->addSuccess($result->getSuccessMessage());
                }   
    ...
    public function serverRedirectAction()
    {
        ...
        if($allowPay){
            ...
            if ($response->getResponseStatus() == Ebizmarts_SagePaySuite_Model_Api_Payment :: RESPONSE_CODE_APPROVED) {
                ...
                Mage::getSingleton("core/session")->addError($response->getResponseStatusDetail());
            }
            $this->getResponse()->setRedirect($returnUrl);
        }else{
    ...
    public function directRedirectAction()
    {
        ...
        if($allowPay){
            ...
            if($response->getResponseStatus() == Ebizmarts_SagePaySuite_Model_Api_Payment::RESPONSE_CODE_3DAUTH && $response->get3DsecureStatus() == "OK"){
                ...
                $this->_redirect("*/*/redirect3D");
            }else{
            
                if($response->getErrorMessage())
                {
                    Mage::getSingleton("core/session")->addError($response->getErrorMessage());
                }
                if($response->getSuccessMessage())
                {
                    Mage::getSingleton("core/session")->addSuccess($response->getSuccessMessage());
                }   
    ...
    public function redirect3DAction(){
        $this->getResponse()->setBody($this->getLayout()->createBlock('md_partialpayment/sagepay_redirect_secure3D')->toHtml());
    }
			...
			$installmentCount = 0;			
                    if ($strStatus == 'OK' || $strStatus == 'AUTHENTICATED' || $strStatus == 'REGISTERED') {
                        if(!$fullFlag){
                            ...
                            $installmentCount = (int)$dueSummaryCollection->count();
    ...
    private function _returnOkAbort($paymentsId=null) {
        ...
        $strResponse .= 'RedirectURL=' . Mage::getUrl("md_partialpayment/sagepay/successRedirect",array("payment_id"=>$paymentsId)) . $this->eoln;
    ...
    private function _returnInvalid($message = 'Unable to find the transaction in our database.',$paymentsId=null) {
        ...
        $response .= 'RedirectURL=' . Mage::getUrl("md_partialpayment/sagepay/failRedirect",array("payment_id"=>$paymentsId)) . $this->eoln;
    ...
    {
        Mage::getSingleton("core/session")->addSuccess(Mage::helper("md_partialpayment")->__('Payment paid successfully.'));
        $this->_redirect("md_partialpayment/summary/view",array("payment_id"=>$this->getRequest()->getParam("payment_id",null)));
        return $this;
    }
    
    public function failRedirectAction()
    {
        Mage::getSingleton("core/session")->addError(Mage::helper("md_partialpayment")->__('Payment failed.'));
        $this->_redirect("md_partialpayment/summary/view",array("payment_id"=>$this->getRequest()->getParam("payment_id",null)));
        return $this;
    }
}