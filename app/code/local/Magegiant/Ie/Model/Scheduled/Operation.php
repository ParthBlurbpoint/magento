 ...
 */
class Magegiant_Ie_Model_Scheduled_Operation extends Mage_Core_Model_Abstract
{
    /**
     * Log directory
     *
     ...
     */
    const CRON_MODEL = 'magegiant_ie/observer::processScheduledOperation';
    ...
    {
        $this->_init('magegiant_ie/scheduled_operation');
    }
     ...
     *
     * @param array $vars
     * @return Magegiant_Ie_Model_Scheduled_Operation
     */
    ...
    public function loadByJobCode($jobCode)
    {
        ...
        if (!isset($operationId) || !$operationId) {
            Mage::throwException(Mage::helper('magegiant_ie')->__('Invalid cron job task'));
        }
    ...
    public function run()
    {
        ...
        if (!file_exists($filePath)) {
            $filePath = Mage::helper('magegiant_ie')->__('File has been not created');
        }

        if (!$result || isset($e) && is_object($e)) {
            $operation->addLogComment(
                Mage::helper('magegiant_ie')->__('Operation finished with fail status')
            );
     ...
     *
     * throws Mage_Core_Exception
     * @param Magegiant_Ie_Model_Scheduled_Operation_Interface $operation
     * @return string full file path
     */
    public function getFileSource(Magegiant_Ie_Model_Scheduled_Operation_Interface $operation)
    {
        $fileInfo = $this->getFileInfo();
        if (empty($fileInfo['file_name'])) {
            Mage::throwException(
                Mage::helper('magegiant_ie')->__('Unable to read file source. File name is empty')
            );
        }
        $operation->addLogComment(Mage::helper('magegiant_ie')->__('Connecting to server'));
        $fs = $this->getServerIoDriver();
        $operation->addLogComment(Mage::helper('magegiant_ie')->__('Reading import file'));
        ...
        if (!$fs->read($filePath, $tmpFilePath)) {
            Mage::throwException(Mage::helper('magegiant_ie')->__('Unable to read import file'));
        }
        $fs->close();
        $operation->addLogComment(
            Mage::helper('magegiant_ie')->__('Save history file content "%s"', $this->getHistoryFilePath())
        );
     ...
     * @return bool
     */
    public function saveFileSource(Magegiant_Ie_Model_Scheduled_Operation_Interface $operation, $fileContent)
    {
        $result = false;
        ...
        if (!$result) {
            Mage::throwException(
                Mage::helper('magegiant_ie')->__('Unable to write file "%s" to "%s" with "%s" driver', $fileName, $fileInfo['file_path'], $fileInfo['server_type'])
            );
        }
        $operation->addLogComment(Mage::helper('magegiant_ie')->__('Save file content'));
     ...
     *
     * @throws Mage_Core_Exception
     * @return Magegiant_Ie_Model_Abstract
     */
    public function getInstance()
    {
        $operation = Mage::getModel('magegiant_ie/' . $this->getOperationType());
        if (!$operation || !($operation instanceof Magegiant_Ie_Model_Scheduled_Operation_Interface)) {
            Mage::throwException(Mage::helper('magegiant_ie')->__('Invalid scheduled operation'));
        }
    ...
    {
        $fileInfo = $this->getFileInfo();
        $availableTypes = Mage::getModel('magegiant_ie/scheduled_operation_data')
            ->getServerTypesOptionArray();
        if (!isset($fileInfo['server_type'])
            || !$fileInfo['server_type']
            || !isset($availableTypes[$fileInfo['server_type']])
        ) {
            Mage::throwException(Mage::helper('magegiant_ie')->__('Invalid server type'));
        }

        $class = 'Varien_Io_' . ucfirst(strtolower($fileInfo['server_type']));
        if (!class_exists($class)) {
            Mage::throwException(
                Mage::helper('magegiant_ie')->__('Invalid server comunication class "%s"', $class)
            );
        }
        $driver = new $class;
    ...
    protected function _saveOperationHistory($source)
    {
        ...
        if (!$fs->write(basename($filePath), $source)) {
            Mage::throwException(Mage::helper('magegiant_ie')->__('Unable to save file history file'));
        }
        return $this;
    }
    ...
    public function getHistoryFilePath()
    {
        ...
        if (isset($fileInfo['file_format'])) {
            ...
            Mage::throwException(Mage::helper('magegiant_ie')->__('Unknown file format'));
        }
	...
	public function  reportFailToEmail($operation)
	{
		...
		$vars       = array(
			...
			'dateAndTime'   => Mage::getModel('core/date')->date(),
			'fileName'      => $fileInfo['file_path'] . DS . $fileInfo['file_name'],
		);
		$storeId    = Mage::app()->getStore()->getId();
		$copyTo     = explode(',', $operation->getEmailCopy());
		$copyMethod = $operation->getEmailCopyMethod();

		$mailer    = Mage::getSingleton('core/email_template_mailer');
		$emailInfo = Mage::getModel('core/email_info');

		$receiverEmail = Mage::getStoreConfig(
			self::CONFIG_PREFIX_EMAILS . $operation->getEmailReceiver() . '/email',
			$storeId
		);
		$receiverName  = Mage::getStoreConfig(
			self::CONFIG_PREFIX_EMAILS . $operation->getEmailReceiver() . '/name',
			$storeId
		);
		...
		// Set all required params and send emails
		$mailer->setSender($operation->getEmailSender());
		$mailer->setStoreId($storeId);
		$mailer->setTemplateId($operation->getEmailTemplate());
		$mailer->setTemplateParams($vars);
		try {
			$mailer->send();
		} catch (Exception $e) {
			Mage::log($e->getMessage(),Magegiant_Ie_Helper_Data::LOG_FILE);
		}
