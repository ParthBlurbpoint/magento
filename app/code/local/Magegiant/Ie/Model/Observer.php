 ...
 */
class Magegiant_Ie_Model_Observer
{
    /**
     * Cron tab expression path
     */
    const CRON_STRING_PATH = 'crontab/jobs/magegiant_import_export_log_clean/schedule/cron_expr';

    /**
     * Configuration path of log status
     */
    const LOG_CLEANING_ENABLE_PATH = 'system/magegiant_import_export_log/enabled';

    /**
     * Configuration path of log save days
     */
    const SAVE_LOG_TIME_PATH = 'system/magegiant_import_export_log/save_days';

    /**
     * Recipient email configuraiton path
     */
    const XML_RECEIVER_EMAIL_PATH = 'system/magegiant_import_export_log/error_email';

    /**
     * Sender email configuraiton path
     */
    const XML_SENDER_EMAIL_PATH   = 'system/magegiant_import_export_log/error_email_identity';

    /**
     * Email template configuraiton path
     */
    const XML_TEMPLATE_EMAIL_PATH = 'system/magegiant_import_export_log/error_email_template';
    ...
    public function scheduledLogClean($schedule, $forceRun = false)
    {
        ...
        try {
            $logPath = Mage::getConfig()->getOptions()->getVarDir() . DS
                     . Magegiant_Ie_Model_Scheduled_Operation::LOG_DIRECTORY;

            if (!file_exists($logPath) || !is_dir($logPath)) {
                if (!mkdir($logPath, 0777, true)) {
                    Mage::throwException(
                        Mage::helper('magegiant_ie')->__('Unable to create directory "%s".', $logPath)
                    );
                }
            }
            ...
            if (!is_dir($logPath) || !is_writable($logPath)) {
                Mage::throwException(
                    Mage::helper('magegiant_ie')->__('Directory "%s" is not writable.', $logPath)
                );
            }
            ...
            foreach ($this->_getDirectoryList($logPath) as $directory) {
                ...
                if ($forceRun || $direcotryDate < $dateCompass) {
                    ...
                    if (!$fs->rmdirRecursive($directory, true)) {
                        ...
                        Mage::throwException(
                            Mage::helper('magegiant_ie')->__('Unable to delete "%s". Directory is not writable.', $directory)
                        );
                    }
                }
            }
     ...
     *
     * @param array $vars
     * @return Magegiant_Ie_Model_Observer
     */
