...
class Kodematix_Socialconnect_Model_Twitter_Applicant
{
    ...
    const XML_PATH_ENABLED = 'customer/kodematix_socialconnect_twitter/enabled';
    const XML_PATH_CLIENT_ID = 'customer/kodematix_socialconnect_twitter/client_id';
    const XML_PATH_CLIENT_SECRET = 'customer/kodematix_socialconnect_twitter/client_secret';
    ...
    public function fetchRequestToken()
    {
        if(!($requestToken = $this->client->getRequestToken())) {
            throw new Exeption(
                Mage::helper('kodematix_socialconnect')
                    ->__('Unable to get request token.')
            );
        }
        ...
        if($response->isError()) {
            ...
            if(($status == 400 || $status == 401 || $status == 429)) {
                if(isset($decoded_response->error->message)) {
                    ...
                    $message = Mage::helper('kodematix_socialconnect')
                        ->__('Unspecified OAuth error occurred.');
                }

                throw new Kodematix_Socialconnect_TwitterOAuthException($message);
            } else {
                $message = sprintf(
                    Mage::helper('kodematix_socialconnect')
...
class Kodematix_Socialconnect_TwitterOAuthException extends Exception
{}
